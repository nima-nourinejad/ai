...
def search_similar_chunks(question, top_k=5):
    ...
    # Handle noise points (DBSCAN assigns -1 for noise)
    noise_points = [idx for idx, cluster in zip(top_chunk_indices, top_clusters) if cluster == -1]
    
    # Group chunks by their clusters, excluding noise points
    cluster_to_chunks = {}
    for idx, cluster in zip(top_chunk_indices, top_clusters):
        if cluster != -1:  # Only consider non-noise clusters
            if cluster not in cluster_to_chunks:
                cluster_to_chunks[cluster] = []
            cluster_to_chunks[cluster].append(sentence_chunks[idx])
    
    # Optionally, you could return both top chunks and noise points separately
    return top_chunks, cluster_to_chunks, noise_points
